...
class SessionLogMessageMiddleware(BaseMiddleware):
    async def __call__(
        ...
        slide: Slide = await get_slide_by_id(lesson_id=session.lesson_id, slide_id=session.current_slide_id, db_session=db_session)
        json_event = event.model_dump_json(exclude_unset=True)
        session_log = SessionLog(session_id=session.id, slide_id=session.current_slide_id, slide_type=slide.slide_type, data=event.text, update=json_event)
...
class SessionLogCallbackMiddleware(BaseMiddleware):
    async def __call__(
        ...
        slide: Slide = await get_slide_by_id(lesson_id=session.lesson_id, slide_id=session.current_slide_id, db_session=db_session)
        json_event = event.model_dump_json(exclude_unset=True)
        session_log = SessionLog(session_id=session_id, slide_id=session.current_slide_id,
                                 slide_type=slide.slide_type, data=answer, update=json_event)
        db_session.add(session_log)
        return await handler(event, data)
