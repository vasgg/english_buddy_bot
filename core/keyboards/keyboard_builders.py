from aiogram.types import InlineKeyboardMarkup
from aiogram.utils.keyboard import InlineKeyboardBuilder

from core.keyboards.callback_builders import QuizCallbackFactory, SlideCallbackFactory


def get_furher_button(current_lesson: int, current_slide: int) -> InlineKeyboardMarkup:
    furter_button_builder = InlineKeyboardBuilder()
    furter_button_builder.button(text='Далее', callback_data=SlideCallbackFactory(lesson_number=current_lesson,
                                                                                  slide_number=current_slide + 1))
    return furter_button_builder.as_markup()


def get_quiz_keyboard(words: list[str], answer: str, lesson_number: int, slide_number: int) -> InlineKeyboardMarkup:
    quiz_keyboard = InlineKeyboardBuilder()
    for word in words:
        if word == answer:
            quiz_keyboard.button(text=word, callback_data=QuizCallbackFactory(answer=word, lesson_number=lesson_number, slide_number=slide_number))
        else:
            quiz_keyboard.button(text=word, callback_data=QuizCallbackFactory(answer='wrong', lesson_number=lesson_number, slide_number=slide_number))
    quiz_keyboard.adjust(1)
    return quiz_keyboard.as_markup()
