...
async def get_lesson_picker_keyboard(
    lessons: list[Lesson], completed_lessons: set[int]
        ...
        [
            InlineKeyboardButton(
                ...
                callback_data=SlideCallbackFactory(
                    lesson_id=current_lesson, next_slide_id=next_slide
                ).pack(),
            )
        ]
    ]
...
def get_quiz_keyboard(
    words: list[str], answer: str, lesson_id: int, slide_id: int
) -> InlineKeyboardMarkup:
    buttons = []
    for word in words:
        if word == answer:
                ...
                [
                    InlineKeyboardButton(
                        ...
                        callback_data=QuizCallbackFactory(
                            answer=word, lesson_id=lesson_id, slide_id=slide_id
                        ).pack(),
                    )
                ]
            )
